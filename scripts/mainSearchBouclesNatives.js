...
export const searchBouclesNatives = (input, arrayRecipes) => {
    ...
    for (let i = 0; i < arrayRecipes.length; i++) {
        ...
        for (let j = 0; j < arrayWordsTitleRecipe.length; j++) {
            let isTrue = comparaison(arrayWordsTitleRecipe[j], input);
            if (isTrue) {
        ...
        for (let k = 0; k < arrayWordsDescriptionRecipe.length; k++) {
            let isTrue = comparaison(arrayWordsDescriptionRecipe[k], input)
            if (isTrue) {
                arrayFilter = [...arrayFilter, recipe];
                break;
            };
        ...
        for (let m = 0; m < ingredientsOfRecipe.length; m++) {
            ...
            for (let n = 0; n < arrayOfIngredientsRecipe.length; n++) {
                let isTrue = comparaison(arrayOfIngredientsRecipe[n], input);
                if (isTrue) {
                    arrayFilter = [...arrayFilter, recipe];
                };
            } 
