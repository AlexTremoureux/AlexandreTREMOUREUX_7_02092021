...
// Recherche des ingrédients
  export const getIngredients = (array) => {
    ...
    return allIngredients.map((element) => ({ type: 'ingredient', name: element }));
  };
  ...
  export const searchIngredientsFunctionTag = (input, arrayIngred) => {
    ...
    let ingredientsFilter = arrayIngred.filter(list => normalize(list.name).includes(input));
  ...
  export const searchDevicesFunctionTag = (input, arrayIngred) => {
    ...
    // Tous les ingrédients qui correspondent à l'input sont mis dans un array
    let devicesFilter = arrayIngred.filter(list => normalize(list.name).includes(input));
  ...
  export const searchUstensilsFunctionTag = (input, arrayIngred) => {
    ...
    let ustensilsFilter = arrayIngred.filter(list => normalize(list.name).includes(input));
